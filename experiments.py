...
def dbpedia():
    train_df = pd.read_csv('data/dbpedia_14/train.csv')
    # Load the test dataset into a pandas dataframe.
    eval_df = pd.read_csv('data/dbpedia_14/test.csv')
    label = "label"
    train_df_new, train_df_discard, _, _ = train_test_split(train_df, train_df.iloc[:,0], train_size=20000, random_state=42, stratify=train_df.iloc[:,0])
    return train_df_new, eval_df, label
...
def get_dataset(dataset):
    if dataset == "sst2":
        ...
        train_df, eval_df, label = trec()
    elif dataset == "dbpedia":
        train_df, eval_df, label = dbpedia()
    else:
        train_df, eval_df, label = imdb()
...
def fit(train_label,train_unlabel, algo="Entropy"):
    ...
    model.fit(train_objectives=[(train_dataloader, train_loss)], epochs=5, warmup_steps=30, show_progress_bar=True)
    ...
    #* num_classes
    print('Acc. SetFit', accuracy_score(y_eval, y_pred_eval_sgd), flush=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--dataset", default=None, required=True, type=str, help="Dataset [sst2, trec6, dbpedia, imdb]")
    parser.add_argument("--acq_fn", default=None, required=True, type=str, help="Acq_fn [Entropy, random]")
    args = parser.parse_args()

    dataset = args.dataset
    acq_fn = args.acq_fn
    
    train_df, eval_df, label = get_dataset(dataset)
    ...
    aquisition_size = len(train_df)//100
    ...
    acq_iter = 15
    results =[[] for i in range(acq_iter)]
    #results_entropy =[[] for i in range(10)]
    print("Acq_Function ", acq_fn, flush=True)
    for j in range(5):
        ...
        for i in range(acq_iter):
            train_label,train_unlabel ,acc= fit(train_label,train_unlabel,acq_fn)
            print(len(train_label), flush=True)
