...
class SupCsTrainer(Trainer):
    ...
    def compute_loss(
        ...
        # ----- Default p = 0.1 ---------#
        output = model(**inputs)
        logits,last_hidden_state_cls = output
        if self.pooling_strategy == 'pooler':
            try:
                embeddings = last_hidden_state_cls.unsqueeze(1)
            except:
                embeddings = output.last_hidden_state.mean(dim=1, keepdim=True)
                print("FATA")
        else:
            embeddings = output.last_hidden_state.mean(dim=1, keepdim=True)
            print("FATA")
        ...
        for p_dpr in self.w_drop_out:
            ...
            if self.pooling_strategy == 'pooler':
                ...
                try:
                    embeddings = torch.cat((embeddings, model(**inputs)[1].unsqueeze(1)), 1)
                except:
                    embeddings = torch.cat((embeddings, model(**inputs).last_hidden_state.mean(dim=1, keepdim=True)), 1)
                    print('FATA')
            else:
                embeddings = torch.cat((embeddings, model(**inputs).last_hidden_state.mean(dim=1, keepdim=True)), 1)
            
        # ---- L2 norm ---------#
        embeddings = FF.normalize(embeddings, p=2, dim=2)
        ...
        loss_supcon = loss_fn(embeddings, labels)

        lamb = 0.9


        loss =  (1-lamb)*loss_ce+ lamb*loss_supcon
    ...
    def set_dropout_mf(
        ...
        if hasattr(model, 'module'):
            ...
            model.bert.embeddings.dropout.p = w
